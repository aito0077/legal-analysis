...
export default function ReportsPage() {
  ...
  const getRiskIntensity = (count: number, maxCount: number) => {
    ...
    if (intensity === 100) return 'bg-destructive text-destructive-foreground font-bold';
    if (intensity >= 75) return 'bg-destructive/80 text-destructive-foreground font-semibold';
      ...
      <DashboardLayout>
        <div className="container mx-auto px-4 py-8">
          <div className="bg-destructive/10 border border-destructive/20 rounded-xl p-8 text-center">
            ...
            <p className="text-destructive font-semibold mb-2">Error al cargar reporte</p>
      ...
      <DashboardLayout>
        <div className="container mx-auto px-4 py-8">
          <div className="bg-card rounded-xl shadow-md p-12 text-center">
            ...
            <h2 className="text-2xl font-bold text-foreground mb-2">No hay datos suficientes</h2>
      ...
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        <div className="mb-8 flex items-center justify-between">
          ...
          <div className="relative">
            <button
              className="flex items-center gap-2 px-6 py-3 bg-primary text-primary-foreground rounded-lg hover:bg-primary/90 shadow-md"
            ...
            {showExportMenu && (
              <div className="absolute right-0 mt-2 w-56 bg-card rounded-lg shadow-xl border z-10">
                <button
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">Reporte completo en formato PDF</p>
                  </div>
                </button>
                <div className="border-t"></div>
                <button
                  onClick={handleExportExcel}
                  className="w-full flex items-center gap-3 px-4 py-3 hover:bg-accent transition-colors text-left rounded-b-lg"
                >
                  <FileSpreadsheet className="h-5 w-5 text-green-600" />
                  <div>
                    <p className="font-medium text-foreground">Exportar Excel</p>
                    <p className="text-xs text-muted-foreground">Datos en hojas de cálculo</p>
                  </div>
                </button>
              </div>
            )}
            ...
            <div className="bg-card rounded-xl shadow-md p-6">
              <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-foreground">{summary.totalRisks}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-card rounded-xl shadow-md p-6">
              <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-foreground">{summary.averageInherentRisk}/25</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-card rounded-xl shadow-md p-6">
              <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-foreground">{summary.riskReduction}%</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-card rounded-xl shadow-md p-6">
              <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-foreground">{controlEffectiveness?.percentage}%</p>
                </div>
              </div>
            </div>
          </div>
        ...
        {riskMatrix && (
          <div className="bg-card rounded-xl shadow-md p-6 mb-8">
            <h2 className="text-2xl font-bold text-foreground mb-6">Matriz de Riesgo 5×5</h2>
              ...
              <div className="inline-block min-w-full">
                <div className="grid grid-cols-6 gap-2">
                  ...
                  {[1, 2, 3, 4, 5].map((impact) => (
                    <div key={`impact-${impact}`} className="flex items-center justify-center text-sm font-semibold text-muted-foreground">
                      {impact === 1 && 'Insignif.'}
                      {impact === 2 && 'Menor'}
                      {impact === 3 && 'Moderado'}
                      {impact === 4 && 'Mayor'}
                  ...
                  {[5, 4, 3, 2, 1].map((likelihood) => (
                    <>
                      {/* Likelihood label */}
                      <div key={`likelihood-${likelihood}`} className="flex items-center justify-end pr-2 text-sm font-semibold text-muted-foreground">
                      ...
                      {[1, 2, 3, 4, 5].map((impact) => {
                        ...
                        return (
                          <div
                            ...
                            className={`aspect-square flex flex-col items-center justify-center rounded-lg border-2 cursor-pointer transition-all hover:scale-105 ${intensityClass}`}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          ...
          {priorityDistribution && (
            <div className="bg-card rounded-xl shadow-md p-6">
              <h3 className="text-xl font-bold text-foreground mb-4">Distribución por Prioridad</h3>
          ...
          {statusDistribution && (
            <div className="bg-card rounded-xl shadow-md p-6">
              <h3 className="text-xl font-bold text-foreground mb-4">Distribución por Estado</h3>
              <div className="space-y-4">
        ...
        {topRisks && topRisks.length > 0 && (
          <div className="bg-card rounded-xl shadow-md p-6">
            <h3 className="text-xl font-bold text-foreground mb-4">Top 10 Riesgos Prioritarios</h3>
            <div className="space-y-3">
              {topRisks.map((risk, index) => (
                <div
                  key={risk.id}
                  onClick={() => router.push(`/dashboard/risks/${risk.id}`)}
                  className="p-4 bg-secondary rounded-lg border hover:border-primary/50 cursor-pointer transition-colors"
                >
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`px-3 py-1 rounded-full text-sm font-medium ${
                      risk.priority === 'CRITICAL' ? 'bg-destructive/10 text-destructive' :
                      risk.priority === 'HIGH' ? 'bg-orange-500/10 text-orange-500' :
                      risk.priority === 'MEDIUM' ? 'bg-yellow-500/10 text-yellow-500' :
